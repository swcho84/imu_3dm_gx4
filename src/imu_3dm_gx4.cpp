...
int main(int argc, char** argv)
{
  ...
  if (enableMagnetometer)
  {
    pubMag = pnh.advertise<sensor_msgs::MagneticField>("/3dm_gx4_25/magnetic_field", 1);
    ROS_INFO("Publish magnetic field to %s", pubMag.getTopic().c_str());
  }
  if (enableFilter)
  {
    pubFilter = pnh.advertise<imu_3dm_gx4::FilterOutput>("/3dm_gx4_25/filter", 1);
    ROS_INFO("Publish filter to %s", pubFilter.getTopic().c_str());
  }
  if (enableTf)
  {
    pubPose = pnh.advertise<geometry_msgs::PoseStamped>("/3dm_gx4_25/pose", 1);
