...
Imu::Packet::Packet(uint8_t desc) : syncMSB(kSyncMSB), syncLSB(kSyncLSB), descriptor(desc), length(0), checksum(0)
{
...
Imu::command_error::command_error(const Packet& p, uint8_t code) : std::runtime_error(generateString(p, code))
{
}
...
Imu::timeout_error::timeout_error(bool write, unsigned int to) : std::runtime_error(generateString(write, to))
{
}

std::string Imu::timeout_error::generateString(bool write, unsigned int to)
{
  std::stringstream ss;
...
Imu::Imu(const std::string& device, bool verbose)
  : device_(device), verbose_(verbose), fd_(0), rwTimeout_(kDefaultTimeout), srcIndex_(0), dstIndex_(0), state_(Idle)
{
...
void Imu::selectBaudRate(unsigned int baud)
{
  ...
  try
  {
    if (verbose_)
    {
      std::cout << "Instructing device to change to " << baud << std::endl << std::flush;
    }
    sendCommand(comm);
  }
...
void Imu::getDiagnosticInfo(Imu::DiagnosticFields& fields, const Imu::Info& info)
{
...
void Imu::setIMUDataRate(uint16_t decimation, const std::bitset<4>& sources)
{
  Imu::Packet p(COMMAND_CLASS_3DM);  //  was 0x04
  PacketEncoder encoder(p);

  //  valid field descriptors: accel, gyro, mag, pressure
  static const uint8_t fieldDescs[] = { FIELD_ACCELEROMETER, FIELD_GYROSCOPE, FIELD_MAGNETOMETER, FIELD_BAROMETER };
...
void Imu::setFilterDataRate(uint16_t decimation, const std::bitset<4>& sources)
{
  ...
  static const uint8_t fieldDescs[] = { FIELD_QUATERNION, FIELD_GYRO_BIAS, FIELD_ANGLE_UNCERTAINTY,
  ...
  else if (state_ == Reading)
  {
    ...
    else if (dstIndex_ == end + 1)
    {
      ...
      if (sum != packet_.checksum)
      {
        ...
        if (verbose_)
        {
          std::cout << "Expected " << std::hex << static_cast<int>(packet_.checksum) << " but received "
